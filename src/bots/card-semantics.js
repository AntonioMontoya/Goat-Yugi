...
const byCode = new Map(OCGCORE_CARD_ENTRIES.map((entry) => {
  ...
  return [Number(entry.runtimeCode), card ? { id: card.id, name: card.name, kind: card.kind, attribute: card.attribute, atk: Number(card.atk) || 0, def: Number(card.def) || 0, level: Number(card.level) || 0, roles: semanticRolesForCard(card) } : null];
}));
